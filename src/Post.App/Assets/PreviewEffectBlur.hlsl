...
float blurRadius : register(c2);   // in texture units

float4 main(float2 uv : TEXCOORD) : COLOR
{
    // A ring of taps approximates the gaussian closely enough to judge the amount.
    // There is no branch: ps_2_0 flattens one and runs out of temp registers.
    float4 source = tex2D(implicitInput, uv) * 2.0;
    source += tex2D(implicitInput, uv + float2(blurRadius, 0));
    source += tex2D(implicitInput, uv - float2(blurRadius, 0));
    source += tex2D(implicitInput, uv + float2(0, blurRadius));
    source += tex2D(implicitInput, uv - float2(0, blurRadius));
    source += tex2D(implicitInput, uv + float2(blurRadius, blurRadius) * 0.7);
    source += tex2D(implicitInput, uv - float2(blurRadius, blurRadius) * 0.7);
    source += tex2D(implicitInput, uv + float2(blurRadius, -blurRadius) * 0.7);
    source += tex2D(implicitInput, uv - float2(blurRadius, -blurRadius) * 0.7);
    source = source / 10.0;
